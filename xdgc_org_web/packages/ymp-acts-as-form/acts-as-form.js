// NOTE: if you want a destroyed callback on your template, you must add it BEFORE mixin this in.
...
ActsAsForm = function(templateName, options) { 
	...
	var fns = {
		...
		updateState : function(name, value) {
			...
			var prev = _.dottedProperty(state,name);
	...
	templ.helpers({
		...
		errorLabel : function(name, title) {
			...
			if (error) {
				var label = '<label class="colortips" for="' + name + '">' + title;
